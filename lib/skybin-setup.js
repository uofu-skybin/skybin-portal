...
console.log('binary path: ' + SKYBIN_BINARY_PATH);
...
function initProvider(options) {
    ...
    if (options.pricingPolicy) {
        args.push('--pricing-policy', options.pricingPolicy);
    }
    if (options.storageRate) {
        args.push('--storage-rate', options.storageRate);
    }
    if (options.minStorageRate) {
        args.push('--min-storage-rate', options.minStorageRate);
    }
    if (options.maxStorageRate) {
        args.push('--max-storage-rate', options.maxStorageRate);
    }
